...
namespace fruit
{
	...
	//椅子坐标 by chen
	const Vec2 TableChairPos[2] = { 
		Vec2(50, 306 - 131),
		Vec2(200, 306 - 90)

	};
	//椅子上人物坐标
	const Vec2 TablePlayerPos[2] = { 
		Vec2(31, 306 - 62),
		Vec2(137, 306 - 38)
	};

	//玩家名字坐标
	const Vec2 TablePlayerNamePos[2] = { 
	...
	const int TableHeadIndex[2] = { 3,1};
	...
	void TableScene::initView()
	{
		...
		auto table = ImageView::create(SETPATH("fruit/table/table_bg.png"));
		...
		// 返回按钮
		auto backbtn = Button::create(SETPATH("fruit/table/btn_back_n.png"), SETPATH("fruit/table/btn_back_d.png"));
		...
		// 快速加入
		auto quickenterbtn = Button::create(SETPATH("fruit/table/btn_quick_enter_n.png"), SETPATH("fruit/table/btn_quick_enter_d.png"));
		...
		for (int i=0; i<nTableCount; i++)
		{
			...
			auto tableview = ImageView::create(SETPATH("fruit/table/table_free.png"));
			tableview->setTag(TAG_TABLE_LIST_INFO+i);
			tableview->setPosition(ccp(tablePosX,tablePosY));
			m_pScrollView->addChild(tableview);

			//桌号
			auto txIndex = TextAtlas::create("0", SETPATH("fruit/table/table_number.png"), 28, 33, "0");
			...
			{
				auto chair = Button::create(SETPATH("fruit/table/chair.png"), SETPATH("fruit/table/chair.png"), SETPATH("fruit/table/chair.png"));
		...
		for (int i = 0; i < tempSize; i++)
		{
			...
			if (itTable != m_mapTableInfo.end())
			{
				...
				if (enterBtn)
				{
					...
					enterBtn->loadTexture(SETPATH("fruit/table/table_free.png"));
			...
			if (it->second.wTableID != INVALID_TABLE)
			{
				...
				if (itTable != m_mapTableInfo.end())
				{
					...
					if (enterBtn)
					{
						...
						if (itTable->second.cbSitDownCount >= 1)
						{
							enterBtn->loadTexture(SETPATH("fruit/table/table_play.png"));
						...
						if (it->second.wChairID != INVALID_CHAIR)
						{
							...
							Sprite* PlayerInfo = Sprite::create(SETPATH("fruit/table/playerinfo.png"));
