...
namespace sszp
{
	...
	bool SettingLayer::init()
	{
		...
		auto pRoot = cocostudio::GUIReader::getInstance()->widgetFromJsonFile(SETPATH("sszp/Setting.json").c_str());
		...
		{
			music_Btn->loadTextureNormal(SETPATH("sszp/sszp/tool/off.png").c_str());
			
		}
		else
		{
			music_Btn->loadTextureNormal(SETPATH("sszp/sszp/tool/no.png").c_str());
		}
		...
		if (!HallSoundManager::instance()->getEffectConfig())//isSound == true是关闭音效
		{
			...
			sound_Btn->loadTextureNormal(SETPATH("sszp/sszp/tool/off.png").c_str());
		}
		else
		{
			sound_Btn->loadTextureNormal(SETPATH("sszp/sszp/tool/no.png").c_str());
		}
