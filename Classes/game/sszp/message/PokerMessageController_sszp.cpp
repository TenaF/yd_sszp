...
#include "PokerMessageController_sszp.h"
	...
	******************************************/
	...
	bool PokerMessageController::hanleLoginFinish(void *buf, WORD wBufLen)
	{
		...
		if (pokerData->getStatus() == US_PLAYING || pokerData->getStatus() == US_READY || pokerData->getStatus() == US_SIT)
		{
			...
			if (!sszp_mainScene)
			{
				sszp_mainScene = MainScene_sszp::createScene();
				CGameClientDlg::getInstance()->sszp_mainScene = sszp_mainScene;
				Director::getInstance()->pushScene(SceneBase::createSceneWithPhysics(sszp_mainScene));
			}
			...
			for (int i = 0; i < GAME_PLAYER; i++)
			{
				...
				if (playerInfo)
				{
					...
					{
						if (sszp_mainScene)
							sszp_mainScene->setOhterPlayer((Object *)playerInfo);
					}
				}
			}
			SendMsg::sendGetSeatUserInfoReq(NetworkManager::FLAG_GAME, UserInfo::instance()->getTableId(), UserInfo::instance()->getSeatId());
	...
	****************/
	...
	bool PokerMessageController::handleUserScore(void *buf, WORD wBufLen)
	{
		...
		if (playerInfo)
		{
			...
			if (sszp_mainScene)
				sszp_mainScene->updateScore(dataBuf);
		}
	...
	bool PokerMessageController::handleUserStatus(void *buf, WORD wBufLen)
	{
		...
		if (dataBuf.dwUserID == UserInfo::instance()->getUserId())
		{
			...
			case US_NULL:
				break;
			case US_FREE:
				if (sszp_mainScene && isGetOut.empty())
				{
					clearMainScene();
				}
				break;
							...
							if (playerInfo)
							{
								if (playerInfo->getTableId() == UserInfo::instance()->getTableId())
								{
									if (sszp_mainScene)
										...
										sszp_mainScene->clearPlayer((Ref *)playerInfo);
								}
			...
			case US_SIT:
			{
						   if (playerInfo)
						   {
							   ...
							   if (playerInfo->getInfo().wTableID == UserInfo::instance()->getTableId())
							   {
								   ...
								   if (sszp_mainScene)
									   sszp_mainScene->setOhterPlayer((Ref *)playerInfo);
							   }
						   }
			}
				break;
	...
	*****************************************************/
	...
	bool PokerMessageController::handleMainFrameSystemMessage(void *buf, WORD wBufLen)
	{
		...
		if (wType&SMT_CLOSE_GAME)
		{
			...
			PromptLayer::promptCommon(PokerMessageController::instance()->getIsGetOut().c_str(), 34, CC_CALLBACK_0(MainScene_sszp::autoExit, sszp_mainScene), PromptLayer::PromptType::TYPE_PROMT_FORCE_CONFIRM);
	...
	bool PokerMessageController::handleGameSceneFree(void *buf, WORD wBufLen)
	{
		...
		if (sszp_mainScene)
		{
			sszp_mainScene->initGameScene(pStatusFree);

			sszp_mainScene->setLabelType(pStatusFree.cbTimeLeave, TimeType_sszp_Leisure);
		}
		return true;
	}
	...
	bool PokerMessageController::handleGameScenePlay(void *buf, WORD wBufLen)
	{
		...
		TimeType_sszp type = TimeType_sszp_none;
		if (pStatusPlay.cbGameStatus == 100)
		{
			type = TimeType_sszp_PlaceJetton;
		}
		else
		{
			type = TimeType_sszp_Start;
		}
		if (sszp_mainScene &&isFirstComeIn)
		{
			sszp_mainScene->initGameScene(pStatusPlay);
			isFirstComeIn = false;
		}
		if (sszp_mainScene)
		{
			sszp_mainScene->setLabelType(pStatusPlay.cbTimeLeave, (TimeType_sszp)type);
		}
		return true;
	}
	...
	void PokerMessageController::onQuitGame()
	{
		...
		if (sszp_mainScene)
		{
			sszp_mainScene->unscheduleAllSelectors();
			sszp_mainScene->stopAllActions();
			sszp_mainScene->removeAllChildren();
			sszp_mainScene->removeFromParentAndCleanup(true);
			sszp_mainScene = nullptr;
			MainScene_sszp::instance = nullptr;
			isFirstComeIn = true;
			CGameClientDlg::getInstance()->sszp_mainScene = nullptr;
		}
	...
	void PokerMessageController::updatePlayerInfo(PokerPlayerInfo *playerInfo)
	{
		...
		case US_FREE:
		{
						...
						{
							if (sszp_mainScene)
								sszp_mainScene->clearPlayer((Object *)playerInfo);
						}
		}
			break;
		case US_READY:
