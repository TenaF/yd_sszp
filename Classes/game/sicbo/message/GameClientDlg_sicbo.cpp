...
#define IDI_ANDROID_BET				1000	
	...
	bool CGameClientDlg::OnSubGameStart(void * pBuffer, WORD wDataSize)
	{
		...
		if (sicbo_mainScene)
		{
			sicbo_mainScene->updateBankerScore(pGameStart.lBankerScore);
		}
		return true;
	}
	...
	bool CGameClientDlg::OnSubGameBet(void * pBuffer, WORD wDataSize)
	{
		...
		if (sicbo_mainScene)
		{
			sicbo_mainScene->setLabelType(pGameStart.cbTimeLeave, TimeType_sicbo_PlaceJetton);
		}
		return true;
	}
	...
	bool CGameClientDlg::OnSubGameFree(void * pBuffer, WORD wDataSize)
	{
		...
		if (sicbo_mainScene)
		{
		sicbo_mainScene->setLabelType(pGameFree.cbTimeLeave, TimeType_sicbo_Leisure);
		}
		return true;
	}
	...
	bool CGameClientDlg::OnSubPlaceJetton(void * pBuffer, WORD wDataSize, bool bGameMes)
	{
		...
		if (sicbo_mainScene)
			sicbo_mainScene->PlaceJettonFunction(pGameFree);
		return true;
	}
	...
	bool CGameClientDlg::OnSubGameEnd(void * pBuffer, WORD wDataSize)
	{
		...
		if (sicbo_mainScene)
		{
			sicbo_mainScene->startMoveBlock(pGameEnd);
			sicbo_mainScene->setLabelType(pGameEnd.cbTimeLeave, TimeType_sicbo_Start);
		}
		return true;
	}
	...
	bool CGameClientDlg::OnSubUserApplyBanker(void * pBuffer, WORD wDataSize)
	{
		...
		if (sicbo_mainScene)
		{
			sicbo_mainScene->userApplyBanker(pApplyBanker->wApplyUser);
		}
		return true;
	}
	...
	bool CGameClientDlg::OnSubUserCancelBanker(void * pBuffer, WORD wDataSize)
	{
		...
		if (sicbo_mainScene)
			sicbo_mainScene->removeBankerSub(pCancelBanker->wCancelUser);
	...
	bool CGameClientDlg::OnSubChangeBanker(void * pBuffer, WORD wDataSize)
	{
		...
		if (sicbo_mainScene)
		{
			sicbo_mainScene->subChangeBanker(pGameEnd);
			sicbo_mainScene->removeBanker(pGameEnd.wBankerUser);
		}
		return true;
	}
	...
	bool CGameClientDlg::OnSubGameRecord(void * pBuffer, WORD wDataSize)
	{
		...
		if (sicbo_mainScene)
		{
			sicbo_mainScene->clearRecordList();
		}
		...
		for (WORD wIndex = 0; wIndex < wRecordCount; wIndex++)
		{
			...
			//***********这里显示路单信息需要修改**********
			sicbo_mainScene->addRecordList(*pServerGameRecord);
		}
		return true;
	}
	...
	bool CGameClientDlg::OnSubPlaceJettonFail(void * pBuffer, WORD wDataSize)
	{
		...
		sicbo_mainScene->hideContinueJetton(true);
	...
	bool CGameClientDlg::OnRepeatPlaceJetton(void * pBuffer, WORD wDataSize)
	{
		...
		if (sicbo_mainScene)
		{
			...
			{
				sicbo_mainScene->userRepeatPlaceJetton(true, pQueryStorage);
			}
			else
			{
				sicbo_mainScene->userRepeatPlaceJetton(false, pQueryStorage);
			}
