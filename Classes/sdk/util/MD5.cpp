...
CMd5::CMd5() {
	memset(m_cPADDING, 0, sizeof(m_cPADDING));
	m_cPADDING[0] = 0x80;
}
...
const unsigned char* CMd5::MakeMD5(const unsigned char* strIn ,int nMD5Len /* = 16 */) {
	Init();
	Update(strIn, nMD5Len);
	Final();
...
void CMd5::Update(const unsigned char *inBuf ,unsigned int inLen) {
	UINT4 in[16];
	int mdi;
	unsigned int i, ii;
	...
	while (inLen--) {
		...
		if (mdi == 0x40) {
			for (i = 0 , ii = 0; i < 16; i++ , ii += 4)
				in[i] = (((UINT4) m_mdContext.in[ii + 3]) << 24) | (((UINT4) m_mdContext.in[ii + 2]) << 16) | (((UINT4) m_mdContext.in[ii + 1]) << 8)
...
void CMd5::Final() {
	...
	for (i = 0 , ii = 0; i < 4; i++ , ii += 4) {
		...
		m_mdContext.digest[ii + 1] = (unsigned char) ((m_mdContext.buf[i] >> 8) & 0xFF);
		m_mdContext.digest[ii + 2] = (unsigned char) ((m_mdContext.buf[i] >> 16) & 0xFF);
		m_mdContext.digest[ii + 3] = (unsigned char) ((m_mdContext.buf[i] >> 24) & 0xFF);
	}
}
