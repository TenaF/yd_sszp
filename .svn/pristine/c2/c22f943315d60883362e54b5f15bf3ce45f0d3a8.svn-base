...
#include "PokerMessageController_fruit.h"
...
namespace fruit
{
#define DELATTIME "DelaytimeRequestedOrientation"
...
#define TAG "PokerMessageController"
	...
	******************************************/
	...
	bool PokerMessageController::hanleLoginFinish(void *buf, WORD wBufLen)
	{
		...
		if (pokerData->getStatus() == US_PLAYING || pokerData->getStatus() == US_READY || pokerData->getStatus() == US_SIT)
		{
			...
			{
				//setScreenH();
				Util::setRequestedOrientation(false);
				fruit_mainScene = MainScene_fruit::createScene();
				//fruit_mainScene->setRotation(270);
				CGameClientDlg::getInstance()->fruit_mainScene = fruit_mainScene;

				Director::getInstance()->pushScene(SceneBase::createSceneWithPhysics(fruit_mainScene));
			}
			...
			for (int i = 0; i < GAME_PLAYER; i++)
			{
				...
				if (playerInfo)
				{
					...
					{
						/*if (fruit_mainScene)
						fruit_mainScene->setOtherPlayer((Object *)playerInfo);*/
					}
				}
			}
	...
	****************/
	...
	bool PokerMessageController::handleUserScore(void *buf, WORD wBufLen)
	{
		...
		if (playerInfo)
		{
			...
			if (fruit_mainScene)
				fruit_mainScene->updateScore(dataBuf);
		}
		...
		else  //其他人
		{
			...
			case US_FREE:
			{
							if (playerInfo)
							{
								...
								{
									//if (fruit_mainScene)
									//fruit_mainScene->clearPlayer((Ref *)playerInfo);
								}
			...
			case US_SIT:
			{
						   if (playerInfo)
						   {
							   ...
							   {
								   /*if (fruit_mainScene)
								   fruit_mainScene->setOtherPlayer((Ref *)playerInfo);*/
							   }
						   }
			}
				break;
	...
	void PokerMessageController::setScreenH()
	{
	...
	*****************************************************/
	...
	bool PokerMessageController::handleGameSceneFree(void *buf, WORD wBufLen)
	{
		...
		if (fruit_mainScene && isFirstComeIn)
		{
			fruit_mainScene->initGameScene(pStatusFree);
			fruit_mainScene->setLabelType(pStatusFree.cbTimeLeave, TimeType_fruit_Leisure);
			isFirstComeIn = false;
		}
		return true;
	}
	...
	bool PokerMessageController::handleGameScenePlay(void *buf, WORD wBufLen)
	{
		...
		TimeType_fruit type = TimeType_fruit_none;
		if (pStatusPlay.cbGameStatus == 101)
		{
			type = TimeType_fruit_PlaceJetton;
		}
		else
		{
			type = TimeType_fruit_Start;
		}

		/*if (fruit_mainScene)
		{
		fruit_mainScene->setLabelType(pStatusPlay.cbTimeLeave, (TimeType_fruit)type);
		}*/
		if (fruit_mainScene &&isFirstComeIn)
		{
			fruit_mainScene->setLabelType(pStatusPlay.cbTimeLeave, (TimeType_fruit)type);
			fruit_mainScene->initGameScene(pStatusPlay);
			isFirstComeIn = false;
		}
	...
	void PokerMessageController::onQuitGame()
	{
		...
		if (fruit_mainScene)
		{
			fruit_mainScene->unscheduleAllSelectors();
			fruit_mainScene->stopAllActions();
			fruit_mainScene->removeAllChildren();
			fruit_mainScene->removeFromParentAndCleanup(true);
			fruit_mainScene = nullptr;
			MainScene_fruit::instance = nullptr;
			CGameClientDlg::getInstance()->fruit_mainScene = nullptr;
		}
		...
		if (myscheduler)
		{
			myscheduler->schedule([&](float dt){
				Director::getInstance()->getScheduler()->unschedule(DELATTIME, this);
				CC_SAFE_DELETE(_instance);
				Util::setRequestedOrientation(true);
			}, this, 0.3f, false, DELATTIME);
		}
	...
	void PokerMessageController::updatePlayerInfo(PokerPlayerInfo *playerInfo)
	{
		...
		case US_FREE:
		{
						...
						{
							//if (fruit_mainScene)
							//fruit_mainScene->clearPlayer((Object *)playerInfo);
						}
		}
			break;
		case US_READY:
	...
	void PokerMessageController::removeGameSceneData()
	{
		...
		if (texture)
		{
		SpriteFrameCache::sharedSpriteFrameCache()->removeSpriteFramesFromFile(SETPATH("hhsw/b_cards/b_card.plist").c_str());
		SpriteFrameCache::sharedSpriteFrameCache()->removeSpriteFramesFromFile(SETPATH("hhsw/s_cards/s_card.plist").c_str());
		for (int i = 1; i < 10; i++)
		{
		SpriteFrameCache::sharedSpriteFrameCache()->removeSpriteFramesFromFile(__String::createWithFormat(SETPATH("hhsw/animation/Bomb/effect_boom_%d.plist").c_str(), i)->getCString());
		}
		}*/
	}
