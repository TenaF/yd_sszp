...
namespace sicbo
{
	...
	bool OutGameLayer::init()
	{
		...
		auto pRoot = cocostudio::GUIReader::getInstance()->widgetFromJsonFile(SETPATH("sicbo/outLayer.json").c_str());
		...
		if (isJesson == true)
		{
			...
			bg->loadTexture(SETPATH("sicbo/exit/db_1.png"));
			bg->setTag(1);
		...
		{
			bg = static_cast<ImageView *>(Panel->getChildByName("bg"));
			bg->loadTexture(SETPATH("sicbo/exit/db.png"));
			bg->setTag(1);
			bg->setTouchEnabled(true);
