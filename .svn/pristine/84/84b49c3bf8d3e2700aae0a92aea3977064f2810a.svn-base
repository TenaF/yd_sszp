#include "MainScene_sicbo.h"
#include "game/sicbo/data/PokerData_sicbo.h"
#include "game/sicbo/message/PokerMessageController_sicbo.h"
#include "game/sicbo/message/SendGameMsg_sicbo.h"
#include "game/sicbo/ui/widget/helpLayer_sicbo.h"
#include "game/sicbo/ui/widget/OutGameLayer_sicbo.h"
#include "game/sicbo/data/SoundManager_sicbo.h"
#include "hall/manager/SoundManager_hall.h"

namespace sicbo
{
	MainScene_sicbo::MainScene_sicbo()
	{
		m_LaterCount = 1;
		isSound = false;
		isSound_music = false;
		...
		wCurrentBankerState = CurrentBanker_sicbo_None;
	...
	void MainScene_sicbo::onEnter()
	{
		SceneBase::onEnter();
		NotificationCenter::getInstance()->addObserver(this, callfuncO_selector(MainScene_sicbo::closeSocket_noifyFial), GAME_DISCONNECT, nullptr);
		SoundManager::instance()->playBgMusic(SETPATH("sicbo/sound/BetBk.mp3").c_str());
	}
	void MainScene_sicbo::onExit()
	{
	...
	void MainScene_sicbo::closeSocket_noifyFial(Object *obj)
	{
	...
	bool MainScene_sicbo::init()
	{
		if (!Layer::init())
		{
			return false;
		}
		pRoot = cocostudio::GUIReader::getInstance()->widgetFromJsonFile(SETPATH("sicbo/sicbao_1.json").c_str());
		pRoot->addTouchEventListener(this, toucheventselector(MainScene_sicbo::onTouch));
		//CCSize visibleSize = Director::getInstance()->getVisibleSize();
		pRoot->setAnchorPoint(CCPoint(0.0f, 0.0f));
		pRoot->setPosition(Vec2(0, 0));
		this->addChild(pRoot);

		over_layout = cocostudio::GUIReader::getInstance()->widgetFromJsonFile(SETPATH("sicbo/sicbao_over.json").c_str());
		over_layout->addTouchEventListener(this, toucheventselector(MainScene_sicbo::onTouch));
		...
		//旋转的圆圈
		S_circleLight = Sprite::create(SETPATH("sicbo/Image/brc_quan.png").c_str());
		...
		B_CurMoney[0]->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onJessonTouch, this));
		B_CurMoney[1] = static_cast<Button *>(BgPanel->getChildByName("cur_2"));
		B_CurMoney[1]->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onJessonTouch, this));
		B_CurMoney[2] = static_cast<Button *>(BgPanel->getChildByName("cur_3"));
		B_CurMoney[2]->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onJessonTouch, this));
		B_CurMoney[3] = static_cast<Button *>(BgPanel->getChildByName("cur_4"));
		B_CurMoney[3]->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onJessonTouch, this));
		B_CurMoney[4] = static_cast<Button *>(BgPanel->getChildByName("cur_5"));
		B_CurMoney[4]->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onJessonTouch, this));
		B_CurMoney[5] = static_cast<Button *>(BgPanel->getChildByName("cur_6"));
		B_CurMoney[5]->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onJessonTouch, this));
		...
		Sprite* sencil = Sprite::create(SETPATH("sicbo/Res/head.png"));
		...
		//动画
		SpriteFrameCache::getInstance()->addSpriteFramesWithFile(SETPATH("sicbo/animation/Yxx0.plist"), SETPATH("sicbo/animation/Yxx0.png"));
		SpriteFrameCache::getInstance()->addSpriteFramesWithFile(SETPATH("sicbo/animation/Yxx1.plist"), SETPATH("sicbo/animation/Yxx1.png"));
		ArmatureDataManager::getInstance()->addArmatureFileInfo(
			SETPATH("sicbo/animation/Yxx0.png"),
			SETPATH("sicbo/animation/Yxx0.plist"),
			SETPATH("sicbo/animation/Yxx.ExportJson"));
		//摇动动画
		...
		Image_77->setVisible(true);
		checkBox = static_cast<CheckBox *>(BgPanel->getChildByName("CheckBox_75"));
		checkBox->setVisible(true);
		checkBox->addEventListenerCheckBox(this, checkboxselectedeventselector(MainScene_sicbo::selectedEvent));
		//---------在线人数，申请上庄---------------------------------
		list_planeL = static_cast<ImageView *>(BgPanel->getChildByName("list_planeL"));
		list_planeL->setVisible(false);
		list_planeL->setZOrder(15);
		list_planeL->setTag(ButtonType_sicbo_listplantL_move);
		list_planeL->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));
		...
		banker_Btn->setTag(ButtonType_sicbo_banker);
		banker_Btn->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));
		
		//侧面申请上庄按钮
		auto banker_move = static_cast<Button *>(list_planeL->getChildByName("banker_move"));
		banker_move->setTag(ButtonType_sicbo_banker_move);
		banker_move->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));
		//---------------左侧结束----------------------
		//路单-----------------------------------------
		ludan_total = static_cast<ImageView *>(BgPanel->getChildByName("ludan_total"));
		ludan_total->setZOrder(15);
		ludan_total->setTag(ButtonType_sicbo_ludan_Frame_move);
		ludan_total->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));

		record_btn = static_cast<Button *>(ludan_total->getChildByName("record_btn"));
		record_btn->setTag(ButtonType_sicbo_ludan_move);
		record_btn->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));
		...
		automaticJ_btn->setTag(ButtonType_sicbo_automaticJ);
		automaticJ_btn->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));
		//跟注次数
		...
		btn_10->setTag(ButtonType_sicbo_automaticJ_10);
		btn_10->addEventListenerCheckBox(this, checkboxselectedeventselector(MainScene_sicbo::selectedEventautomatic));

		auto btn_20 = static_cast<CheckBox *>(btn_plane->getChildByName("btn_20"));
		btn_20->setTag(ButtonType_sicbo_automaticJ_20);
		btn_20->addEventListenerCheckBox(this, checkboxselectedeventselector(MainScene_sicbo::selectedEventautomatic));

		auto btn_50 = static_cast<CheckBox *>(btn_plane->getChildByName("btn_50"));
		btn_50->setTag(ButtonType_sicbo_automaticJ_Infinite);
		btn_50->addEventListenerCheckBox(this, checkboxselectedeventselector(MainScene_sicbo::selectedEventautomatic));
		//重复下注
		repeatJ_btn = static_cast<Button *>(BgPanel->getChildByName("repeatJ_btn"));//重复下注按钮
		repeatJ_btn->setTag(ButtonType_sicbo_repeatJ);
		repeatJ_btn->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));

		//设置帮助退出按钮
		auto menuPanel = static_cast<Widget *>(BgPanel->getChildByName("menuPanel"));
		menuPanel->setZOrder(16);
		...
		menu_btn->setTag(ButtonType_sicbo_menu);
		menu_btn->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));

		auto setting_btn = static_cast<Button *>(menuPanel->getChildByName("setting_btn"));//设置按钮
		setting_btn->setTag(ButtonType_sicbo_setting);
		setting_btn->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));

		auto setting_music = static_cast<Button *>(menuPanel->getChildByName("setting_btn_0"));//设置音乐按钮
		setting_music->setTag(ButtonType_sicbo_settingMusic);
		setting_music->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));
		//根据原来的音效设置声音
		isSound_music = HallSoundManager::instance()->getBgMusicConfig();
		if (isSound_music)
		{
			SoundManager::instance()->playBgMusic(SETPATH("sicbo/sound/BetBk.mp3").c_str());
			setting_music->loadTextures(SETPATH("sicbo/menu/menu_music.png"), SETPATH("sicbo/menu/menu_music_2.png"));
		}
		else
		{
			SoundManager::instance()->stopBgMusic();
			setting_music->loadTextures(SETPATH("sicbo/menu/menu_music3.png"), SETPATH("sicbo/menu/menu_music3_2.png"));
		}
		//---------------------
		isSound = HallSoundManager::instance()->getEffectConfig();
		if (isSound)
		{
			setting_btn->loadTextures(SETPATH("sicbo/menu/btn_set.png"), SETPATH("sicbo/menu/btn_set_2.png"));
		}
		else
		{
			setting_btn->loadTextures(SETPATH("sicbo/menu/menu_sound3.png"), SETPATH("sicbo/menu/menu_sound3_2.png"));
		}
		auto help_btn = static_cast<Button *>(menuPanel->getChildByName("help_btn"));//帮助按钮
		help_btn->setTag(ButtonType_sicbo_help);
		help_btn->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));

		auto out_btn = static_cast<Button *>(menuPanel->getChildByName("out_btn"));//退出按钮
		out_btn->setTag(ButtonType_sicbo_out);
		out_btn->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));

		auto m_bImage_7 = static_cast<ImageView *>(menuPanel->getChildByName("Image_7"));//退出按钮蒙版
		m_bImage_7->setVisible(false);

		auto online_number = static_cast<Text *>(BgPanel->getChildByName("Label_9"));
		online_number->setString("0");
		return true;
	}
	void MainScene_sicbo::closeLight(float dt)
	{
	...
	void MainScene_sicbo::selectedEvent(cocos2d::Ref *pSender, CheckBoxEventType type)
	{
		switch (type) {
	...
	//自动跟注回调事件
	void MainScene_sicbo::selectedEventautomatic(cocos2d::Ref *pSender, CheckBoxEventType type)
	{
		...
		case cocos2d::ui::CHECKBOX_STATE_EVENT_SELECTED:
			if (checkAutoMatic())
			{
				...
				{
				case ButtonType_sicbo_automaticJ_10:
					btn_20->setSelectedState(false);
					btn_50->setSelectedState(false);
					break;
				case ButtonType_sicbo_automaticJ_20:
					btn_10->setSelectedState(false);
					btn_50->setSelectedState(false);
					break;
				case ButtonType_sicbo_automaticJ_Infinite:
					btn_10->setSelectedState(false);
					btn_20->setSelectedState(false);
					break;
				default:
	...
	void MainScene_sicbo::closeBlock()
	{
		for (int itemp_initLightNumber = 0; itemp_initLightNumber < 52; itemp_initLightNumber++)
		{
			blockLight[itemp_initLightNumber]->setVisible(false);
		}
	}
	void MainScene_sicbo::initLight()
	{
		const BlendFunc blendFunc = { GL_ONE, GL_ONE };//去黑色发亮处理
		jettonLabelVec.reserve(52);
		...
		for (int itemp_initLightNumber = 100; itemp_initLightNumber < 152; itemp_initLightNumber++)
		{
			...
			itempImage->addTouchEventListener(CC_CALLBACK_2(MainScene_sicbo::onTouch, this));
			itempImage->setOpacity(0);
			//下注后的标签
			blockLight[itemp_initLightNumber - 100] = ImageView::create(SETPATH("sicbo/Image/score_back.png"));
			...
			ttf->setFntFile(SETPATH("sicbo/fnt/jetton_gold.fnt"));
			...
			myJettonLabel[itemp_initLightNumber - 100]->setFntFile(SETPATH("sicbo/fnt/myJetton_gold.fnt"));
	...
	MainScene_sicbo * MainScene_sicbo::instance = nullptr;
	MainScene_sicbo * MainScene_sicbo::createScene()
	{
		if (!instance)
		{
			instance = MainScene_sicbo::create();
		}
		return instance;
	}

	void MainScene_sicbo::LightPart()
	{
		...
		char mid_itemp = init_ResultNumber[0] + init_ResultNumber[1] + init_ResultNumber[2];
		ImageView *up_smallOrBig = nullptr;
		ImageView *up_singleOrDouble = nullptr;

		//总点数
		auto mid_X = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("mid_%d", mid_itemp)->getCString()));
		mid_X->setOpacity(170);
		mid_X->runAction(RepeatForever::create(seq));

		//总点数比大小
		if (mid_itemp > 10)
		{
			up_smallOrBig = static_cast<ImageView *>(BgPanel->getChildByName("up_big"));
		}
		else
		{
			up_smallOrBig = static_cast<ImageView *>(BgPanel->getChildByName("up_small"));
		}
		up_smallOrBig->setOpacity(170);
		up_smallOrBig->runAction(RepeatForever::create(seq1));

		//总点数比单双
		if (mid_itemp % 2 == 0)
		{
			up_singleOrDouble = static_cast<ImageView *>(BgPanel->getChildByName("up_double"));
		}
		else
		{
			up_singleOrDouble = static_cast<ImageView *>(BgPanel->getChildByName("up_single"));
		}
		up_singleOrDouble->setOpacity(170);
		up_singleOrDouble->runAction(RepeatForever::create(seq2));
	}
	//开始转动转盘
	void MainScene_sicbo::startMoveBlock(CMD_S_GameEnd& gameEnd)
	{
		StartMovesound = 0;
		...
		schedule(schedule_selector(MainScene_sicbo::startDelayMove), 0.0f, 0, 1.0f);
	}

	//开始转动转盘
	void MainScene_sicbo::startDelayMove(float dt)
	{
		//筛子声音
		SoundManager::instance()->playEffect(SETPATH("sicbo/sound/shake_dice.mp3").c_str());
		...
		{
			skin[itemp_shaizi] = Skin::create(SETPATH(__String::createWithFormat("sicbo/shaizi/shaizi_%d_1.png", init_ResultNumber[itemp_shaizi])->getCString()).c_str());
		...
		//筛子声音，豹子报两个声音，其他报5个声音
		if (init_ResultNumber[0] == init_ResultNumber[1] && init_ResultNumber[0] == init_ResultNumber[2])
		{
			schedule(schedule_selector(MainScene_sicbo::soundsStartMove), 1.0f, 1, 5.0f);
		}
		else
		{
			schedule(schedule_selector(MainScene_sicbo::soundsStartMove), 1.0f, 4, 5.0f);

		}

		//开奖区域
		schedule(schedule_selector(MainScene_sicbo::moveEndPanel), 0.0f, 0, 6.0f);
	...
	//随机的角度
	Vec2 MainScene_sicbo::randAngel(int jettonArea)
	{
		switch (jettonArea)
		{
		case 0:
	...
	//报声音
	void MainScene_sicbo::soundsStartMove(float dt)
	{
		//豹子报围晒，三个n，其他的挨个报
		if (init_ResultNumber[0] == init_ResultNumber[1] && init_ResultNumber[0] == init_ResultNumber[2])
		{
			if (StartMovesound == 1)
			{
				SoundManager::instance()->playEffect(__String::createWithFormat(SETPATH("sicbo/sound/san_%d.mp3").c_str(), init_ResultNumber[0])->getCString());
			}
			else
			{
				SoundManager::instance()->playEffect(SETPATH("sicbo/sound/weisai.mp3").c_str());
			}
			StartMovesound++;
		}
		else
		{
			if (StartMovesound == 3)
			{
				int itemp_totalNumber = init_ResultNumber[0] + init_ResultNumber[1] + init_ResultNumber[2];
				SoundManager::instance()->playEffect(__String::createWithFormat(SETPATH("sicbo/sound/dian_%d.mp3").c_str(), itemp_totalNumber)->getCString());
			}
			else if (StartMovesound == 4)
			{
				int itemp_totalNumber = init_ResultNumber[0] + init_ResultNumber[1] + init_ResultNumber[2];
				if (itemp_totalNumber > 10)
				{
					SoundManager::instance()->playEffect(SETPATH("sicbo/sound/da.mp3").c_str());
				}
				else
				{
					SoundManager::instance()->playEffect(SETPATH("sicbo/sound/xiao.mp3").c_str());
				}
			}
			else
			{
				SoundManager::instance()->playEffect(__String::createWithFormat(SETPATH("sicbo/sound/point_%d.mp3").c_str(), init_ResultNumber[StartMovesound])->getCString());
			}
			StartMovesound++;
		}
	...
	void MainScene_sicbo::moveEndPanel(float dt)
	{
		//下注区域的渐隐渐现
		Sequence *seq = Sequence::createWithTwoActions(FadeOut::create(0.7), FadeIn::create(1.0));
		Sequence *seq1 = Sequence::createWithTwoActions(FadeOut::create(0.7), FadeIn::create(1.0));
		...
		{
			//各个点数分别显示
			auto down_X = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_%d", init_ResultNumber[0])->getCString()));
			auto down_Y = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_%d", init_ResultNumber[1])->getCString()));
			auto down_Z = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_%d", init_ResultNumber[2])->getCString()));
			down_X->setOpacity(170);
			down_Y->setOpacity(170);
			down_Z->setOpacity(170);
			...
			{
				down_up_First = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_up_%d_%d", init_ResultNumber[1], init_ResultNumber[0])->getCString()));
			}
			else
			{
				down_up_First = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_up_%d_%d", init_ResultNumber[0], init_ResultNumber[1])->getCString()));
			}
			down_up_First->setOpacity(170);
			...
			{
				down_up_Second = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_up_%d_%d", init_ResultNumber[2], init_ResultNumber[0])->getCString()));
			}
			else
			{
				down_up_Second = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_up_%d_%d", init_ResultNumber[0], init_ResultNumber[2])->getCString()));
			}
			down_up_Second->setOpacity(170);
			...
			{
				down_up_Third = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_up_%d_%d", init_ResultNumber[2], init_ResultNumber[1])->getCString()));
			}
			else
			{
				down_up_Third = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_up_%d_%d", init_ResultNumber[1], init_ResultNumber[2])->getCString()));
			}
			down_up_Third->setOpacity(170);
		...
		else if (init_ResultNumber[0] != init_ResultNumber[1])
		{
			...
			//double双显示
			ImageView *up_double_itemp = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("up_double_%d", init_ResultNumber[2])->getCString()));
			up_double_itemp->setOpacity(170);
			up_double_itemp->runAction(RepeatForever::create(seq4));
		}
		else if (init_ResultNumber[0] != init_ResultNumber[2])
		{
			...
			if (init_ResultNumber[0] > init_ResultNumber[2])
			{
				down_up_First = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_up_%d_%d", init_ResultNumber[2], init_ResultNumber[0])->getCString()));
			}
			else
			{
				down_up_First = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_up_%d_%d", init_ResultNumber[0], init_ResultNumber[2])->getCString()));
			}
			down_up_First->setOpacity(170);
			down_up_First->runAction(RepeatForever::create(seq2));
			
			//double双显示
			ImageView *up_double_itemp = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("up_double_%d", init_ResultNumber[1])->getCString()));
			up_double_itemp->setOpacity(170);
			up_double_itemp->runAction(RepeatForever::create(seq3));
		}
		else if (init_ResultNumber[1] != init_ResultNumber[2])
		{
			...
			if (init_ResultNumber[1] > init_ResultNumber[2])
			{
				down_up_First = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_up_%d_%d", init_ResultNumber[2], init_ResultNumber[1])->getCString()));
			}
			else
			{
				down_up_First = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_up_%d_%d", init_ResultNumber[1], init_ResultNumber[2])->getCString()));
			}
			down_up_First->setOpacity(170);
			down_up_First->runAction(RepeatForever::create(seq2));
			
			//double双显示
			ImageView *up_double_itemp = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("up_double_%d", init_ResultNumber[0])->getCString()));
			up_double_itemp->setOpacity(170);
			up_double_itemp->runAction(RepeatForever::create(seq3));
		}
		else
		{
			//显示最底下的豹子
			auto down_X = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("down_%d", init_ResultNumber[1])->getCString()));
			down_X->setOpacity(170);
			down_X->runAction(RepeatForever::create(seq));
			
			//总点数
			char mid_itemp = init_ResultNumber[0] + init_ResultNumber[1] + init_ResultNumber[2];
			if (init_ResultNumber[0] != 1 && init_ResultNumber[0] != 6)
			{
				auto mid_X = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("mid_%d", mid_itemp)->getCString()));
				mid_X->setOpacity(170);
				mid_X->runAction(RepeatForever::create(seq1));
			}
			
			//double双显示
			ImageView *up_double_itemp = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("up_double_%d", init_ResultNumber[0])->getCString()));
			up_double_itemp->setOpacity(170);
			up_double_itemp->runAction(RepeatForever::create(seq2));
			
			//显示三倍和总三倍
			auto up_triple = static_cast<ImageView *>(BgPanel->getChildByName(__String::createWithFormat("up_triple_%d", init_ResultNumber[1])->getCString()));
			auto up_tripleAll = static_cast<ImageView *>(BgPanel->getChildByName("up_triple_all"));
			up_triple->setOpacity(170);
			up_triple->runAction(RepeatForever::create(seq3));
			up_tripleAll->setOpacity(170);
			up_tripleAll->runAction(RepeatForever::create(seq4));
		}
	}

	void MainScene_sicbo::onTouch(Ref* target, Widget::TouchEventType type)
	{
		...
		if (Widget::TouchEventType::BEGAN == type)
		{
			if (isBanerMove == true)
			{
				if (itemp_tag != ButtonType_sicbo_listplantL_move&&itemp_tag != ButtonType_sicbo_banker && itemp_tag != ButtonType_sicbo_banker_move&&itemp_tag != ButtonType_sicbo_ludan_Frame_move&&itemp_tag != ButtonType_sicbo_ludan_move)
				{
			...
			if (itemp_tag == ButtonType_sicbo_out || itemp_tag == ButtonType_sicbo_help || itemp_tag == ButtonType_sicbo_settingMusic || itemp_tag == ButtonType_sicbo_setting)
			{
				auto menuPanel = static_cast<Widget *>(BgPanel->getChildByName("menuPanel"));
				auto m_bImage_7 = static_cast<ImageView *>(menuPanel->getChildByName("Image_7"));//退出按钮蒙版
				auto m_itempButton = (ImageView *)menuPanel->getChildByTag(itemp_tag);
				m_bImage_7->setPosition(m_itempButton->getPosition() + Vec2(-2, 0));
				m_bImage_7->setVisible(true);
			}
		}
		else if (Widget::TouchEventType::MOVED == type)
		{
			auto menu_panel = (Widget *)BgPanel->getChildByName("menuPanel");
			Vec2 m_vTouchPoint = menu_panel->convertToNodeSpace(btn->getTouchMovePos());
			bool m_bIsTouchUse = btn->getBoundingBox().containsPoint(m_vTouchPoint);
			if (!m_bIsTouchUse)
			{
		...
		else if (Widget::TouchEventType::ENDED == type)
		{
			...
			switch (btn->getTag())
			{
			case ButtonType_sicbo_menu:
			{
										  setMenuState(isMenu);
			}
				break;
			case ButtonType_sicbo_banker_move:
			{
												 setListState(isBanerMove);
			}
				break;
			case ButtonType_sicbo_ludan_move:
			{
												setludanState(isLudanMove);
			}
				break;
			case ButtonType_sicbo_banker:
			{
											if (!isAllowBank)
											{
											...
											{
											case CurrentBanker_sicbo_Already://当前已经是庄，我要下装
											{
																				 SendGameMsg::sendApplyBanker(false);
											}
												break;
											case CurrentBanker_sicbo_Ready://当前是准备上庄，申请上庄
											{
																			   SendGameMsg::sendApplyBanker(false);
											}
												break;
											case CurrentBanker_sicbo_None://没有状态，取消上庄
											{
			...
			case ButtonType_sicbo_repeatJ:
			{
											 repaetButtonNoifyFial();
			}
				break;
			case ButtonType_sicbo_continue:
			{
											  over_layout->setVisible(false);
			}
				break;
			case ButtonType_sicbo_setting:
			{
											 menuButtonMask(false);
											 isSound = HallSoundManager::instance()->getEffectConfig();
				...
				break;
			case ButtonType_sicbo_settingMusic:
			{
												  menuButtonMask(false);
												  isSound_music = HallSoundManager::instance()->getBgMusicConfig();
												  HallSoundManager::instance()->setBgMusicConfig(!isSound_music);
												  auto menuPanel = static_cast<Widget *>(BgPanel->getChildByName("menuPanel"));
												  auto setting_btn = static_cast<Button *>(menuPanel->getChildByName("setting_btn_0"));
												  if (isSound_music)
												  {
													  SoundManager::instance()->stopBgMusic();
													  setting_btn->loadTextures(SETPATH("sicbo/menu/menu_music3.png"), SETPATH("sicbo/menu/menu_music3_2.png"));
												  }
												  else
												  {
													  SoundManager::instance()->playBgMusic(SETPATH("sicbo/sound/BetBk.mp3").c_str());
													  setting_btn->loadTextures(SETPATH("sicbo/menu/menu_music.png"), SETPATH("sicbo/menu/menu_music_2.png"));
												  }
			}
				break;
			case ButtonType_sicbo_help:
			{
										  menuButtonMask(false);
										  helpLayer::createObject();
			}
				break;
			case ButtonType_sicbo_out:
			{
										 menuButtonMask(false);
										 bool istrue = false;
										 if (wCurrentTimeType == TimeType_sicbo_PlaceJetton && my_AllJettonScore != 0)
										 {
											 istrue = true;
										 }
				...
				break;
			case ButtonType_sicbo_automaticJ:
			{
												...
												{
													automaticJ_btn->loadTextures(SETPATH("sicbo/Res/automaticJ_btn_n.png"), "");
													h = -btn_plane->getContentSize().height + 194;
												}
												else
												{
													automaticJ_btn->loadTextures(SETPATH("sicbo/Res/c_automaticJ_btn_n.png"), "");
													h = btn_plane->getContentSize().height - 194;
												}
	...
	//设置menu上的蒙版改变状态
	void MainScene_sicbo::menuButtonMask(bool m_isTrue)
	{
		auto menuPanel = static_cast<Widget *>(BgPanel->getChildByName("menuPanel"));
		auto m_bImage_7 = static_cast<ImageView *>(menuPanel->getChildByName("Image_7"));//退出按钮蒙版
		m_bImage_7->setVisible(m_isTrue);
	}

	void MainScene_sicbo::JettonArea(Button *btn)
	{
		if (!strcmp(bankerUserL->getString().c_str(), UserInfo::instance()->getNickName().c_str()))
		{
		...
		if (wCurrentTimeType != TimeType_sicbo_PlaceJetton)
		{
	...
	//押注限制目前这里没用到
	bool MainScene_sicbo::PlaceJettonLimit(int type)
	{
	...
	void MainScene_sicbo::clearPlayer(Object * player)
	{
	...
	//设置筹码上的金额
	void MainScene_sicbo::setJessonValue(const LONGLONG itemp_Chip[])
	{
		//筹码保存值
		for (int i = 0; i < 6; i++)
		{
	...
	void MainScene_sicbo::initGameScene(CMD_S_StatusFree *obj)
	{
		...
		//设置筹码的金额
		LONGLONG itempChip[6] = { 0 };
		memcpy(&itempChip, obj->nChip, sizeof(itempChip));
		this->setJessonValue(itempChip);
		...
		{
			wCurrentBankerState = CurrentBanker_sicbo_Already;
			this->changeBankerButton();
			this->showUpeerBanker(true);
		}
	}

	void MainScene_sicbo::initGameScene(CMD_S_StatusPlay *obj)
	{
		PokerPlayerInfo * player = PokerData::instance()->getPlayerInfoBySeatId(obj->wBankerUser);
		auto list_planeL = static_cast<Widget *>(BgPanel->getChildByName("list_planeL"));
		auto list_banker = static_cast<Text *>(list_planeL->getChildByName("list_banker"));
		...
		if (wCurrentBanker == UserInfo::instance()->getSeatId())
		{
			wCurrentBankerState = CurrentBanker_sicbo_Already;
			banker_Btn->setTouchEnabled(false);
			banker_Btn->loadTextureNormal(SETPATH("sicbo/Image/banker_Btn2_s.png").c_str());
			this->showUpeerBanker(true);
		...
		{
			auto itemp_shaizi_1 = Sprite::create(SETPATH(__String::createWithFormat("sicbo/shaizi/shaizi_%d_1.png", obj->cbDiceValue[0])->getCString()).c_str());
			itemp_shaizi_1->setName("itemp_shaizi");
			itemp_shaizi_1->setPosition(1097, 725);
			BgPanel->addChild(itemp_shaizi_1);

			auto itemp_shaizi_2 = Sprite::create(SETPATH(__String::createWithFormat("sicbo/shaizi/shaizi_%d_1.png", obj->cbDiceValue[1])->getCString()).c_str());
			itemp_shaizi_2->setName("itemp_shaizi");
			itemp_shaizi_2->setPosition(1168, 725);
			BgPanel->addChild(itemp_shaizi_2);

			auto itemp_shaizi_3 = Sprite::create(SETPATH(__String::createWithFormat("sicbo/shaizi/shaizi_%d_1.png", obj->cbDiceValue[2])->getCString()).c_str());
		...
		{
			if (wCurrentTimeType == TimeType_sicbo_Start)
			{
	...
	void MainScene_sicbo::changeBankerButton()
	{
		switch (wCurrentBankerState)
		{
		case CurrentBanker_sicbo_Already:
		{
											banker_Btn->loadTextureNormal(SETPATH("sicbo/Image/banker_Btn2.png"));
		}
			break;
		case CurrentBanker_sicbo_Ready:
		{
										  banker_Btn->loadTextureNormal(SETPATH("sicbo/Image/banker_Btn3.png"));
		}
			break;
		case CurrentBanker_sicbo_None:
		{
										 banker_Btn->loadTextureNormal(SETPATH("sicbo/onlineFrame/banker_Btn1.png"));
		}
			break;
		default:
			break;
	...
	void MainScene_sicbo::setOtherPlayer(Object * player)
	{
		PokerPlayerInfo* Player = (PokerPlayerInfo*)player;

		for (int i = 0; i < ohter_list->getItems().size(); i++)
		{
			if (!strcmp(ohter_list->getItem(i)->getName().c_str(), Player->getNickName().c_str()))
			{
				LabelTTF * ttf = (LabelTTF *)ohter_list->getItem(i)->getChildByName(Player->getNickName().c_str())->getChildByName("scoreTTf");
				if (ttf)
				{
				...
				for (int j = 0; j < banker_list->getItems().size(); j++)
				{
					...
					if (!strcmp(banker_list->getItem(j)->getName().c_str(), ohter_list->getItem(i)->getName().c_str()))
					{
						...
						if (ttf_bank)
						{
							ttf_bank->setString(__String::createWithFormat("%lld", Player->getScore())->getCString());
						}
						return;
					}
				}
		...
		auto banker = Sprite::create(SETPATH("sicbo/Image/playerList.png"));
		banker->setName(Player->getNickName().c_str());
		auto bg = Layout::create();
		bg->setContentSize(banker->getContentSize());
		bg->setTag(Player->getSeatId());
		bg->setName(Player->getNickName().c_str());
		banker->setAnchorPoint(Vec2::ZERO);
		bg->addChild(banker); int faceId = Player->getFaceId();
		...
		auto gold = Sprite::create(SETPATH("sicbo/Image/gold.png"));
		gold->setScale(0.4f);
		gold->setPosition(Vec2(-15, 0) + scoreTTf->getPosition());
		banker->addChild(gold);
		if (Player->getInfo().dwUserID != UserInfo::instance()->getUserId())
		{
	...
	void MainScene_sicbo::updateScore(CMD_GR_MobileUserScore& buf)
	{
	...
	//系统坐庄时金币更新
	void MainScene_sicbo::updateBankerScore(SCORE itemp_bankerScore)
	{
	...
	void MainScene_sicbo::createUpdate(int time)
	{
		timeLabelTop->setString(__String::createWithFormat("%d%s", time, "s")->getCString());
		timeLabelTop->setVisible(true);
		schedule(schedule_selector(MainScene_sicbo::timeUpdate), 1.0f);
	}
	void MainScene_sicbo::timeUpdate(float dt)
	{
		...
		if (num > 1)
		{
			...
			if (timeLabelTop)
			{
				...
				if (num <= 4 && wCurrentTimeType == TimeType_sicbo_PlaceJetton)
				{
					SoundManager::instance()->playEffect(SETPATH("sicbo/sound/time_waring.mp3").c_str());
				}
			}
		}
		else
	...
	void MainScene_sicbo::removeTimeLabel()
	{
		unschedule(schedule_selector(MainScene_sicbo::timeUpdate));
		timeLabelTop->setString(__String::createWithFormat("%d%s", 0, "s")->getCString());
	}
	void MainScene_sicbo::PlaceJettonFunction(CMD_S_PlaceJetton& jetton)
	{
		SoundManager::instance()->playEffect(SETPATH("sicbo/sound/add_gold.mp3").c_str());//这里添加声音
	...
	void MainScene_sicbo::updateShowScore(bool seatId, BYTE jettonArea, SCORE jettonScore, bool isFirstBet)
	{
		if (seatId)//如果是自己
		{
			...
			//处理筹码的金额
			if (itemp_myJettonLabel[jettonArea] >= 10000 && itemp_myJettonLabel[jettonArea] < 10000000)
			{
				double itemp_isInteger = itemp_myJettonLabel[jettonArea] / 10000;
				if (itemp_isInteger == (int)itemp_isInteger)
				{
					myJettonLabel[jettonArea]->setString(__String::createWithFormat("%g%s", itemp_myJettonLabel[jettonArea] / 10000, U2F("万").c_str())->getCString());
				}
				else
				{
					myJettonLabel[jettonArea]->setString(__String::createWithFormat("%.2f%s", itemp_myJettonLabel[jettonArea] / 10000, U2F("万").c_str())->getCString());
				}

			}
			else if (itemp_myJettonLabel[jettonArea] >= 10000000 && itemp_myJettonLabel[jettonArea] < 100000000)
			{
				double itemp_isInteger = itemp_myJettonLabel[jettonArea] / 10000;
				if (itemp_isInteger == (int)itemp_isInteger)
				{
				...
				{
					myJettonLabel[jettonArea]->setString(__String::createWithFormat("%.1f%s", itemp_myJettonLabel[jettonArea] / 10000, U2F("万").c_str())->getCString());
				}

			}
			else if (itemp_myJettonLabel[jettonArea] >= 100000000 && itemp_myJettonLabel[jettonArea] < 10000000000)
			{
				double itemp_isInteger = itemp_myJettonLabel[jettonArea] / 100000000;
				if (itemp_isInteger == (int)itemp_isInteger)
				{
					myJettonLabel[jettonArea]->setString(__String::createWithFormat("%g%s", itemp_myJettonLabel[jettonArea] / 100000000, U2F("亿").c_str())->getCString());
				}
				else
				{
					myJettonLabel[jettonArea]->setString(__String::createWithFormat("%.3f%s", itemp_myJettonLabel[jettonArea] / 100000000, U2F("亿").c_str())->getCString());
				}

			}
			else
			{
				myJettonLabel[jettonArea]->setString(__String::createWithFormat("%g", itemp_myJettonLabel[jettonArea])->getCString());
			}
			//每次下注检查筹码显示
			if (isContinueJetton == false && wCurrentTimeType == TimeType_sicbo_PlaceJetton)
			{
				for (int i = 0; i < 6; i++)
				{
		...
		if (itemp_totalTtf[jettonArea] >= 10000 && itemp_totalTtf[jettonArea] < 10000000)
		{
			double itemp_isInteger = itemp_totalTtf[jettonArea] / 10000;
			if (itemp_isInteger == (int)itemp_isInteger)
			{
				ttf->setString(__String::createWithFormat("%g%s", itemp_totalTtf[jettonArea] / 10000, U2F("万").c_str())->getCString());
			}
			else
			{
				ttf->setString(__String::createWithFormat("%.2f%s", itemp_totalTtf[jettonArea] / 10000, U2F("万").c_str())->getCString());
			}
		}
		else if (itemp_totalTtf[jettonArea] >= 10000000 && itemp_totalTtf[jettonArea] < 100000000)
		{
			double itemp_isInteger = itemp_totalTtf[jettonArea] / 10000;
			if (itemp_isInteger == (int)itemp_isInteger)
			{
			...
			{
				ttf->setString(__String::createWithFormat("%.1f%s", itemp_totalTtf[jettonArea] / 10000, U2F("万").c_str())->getCString());
			}
		}
		else if (itemp_totalTtf[jettonArea] >= 100000000 && itemp_totalTtf[jettonArea] < 10000000000)
		{
			double itemp_isInteger = itemp_totalTtf[jettonArea] / 100000000;
			if (itemp_isInteger == (int)itemp_isInteger)
			{
				ttf->setString(__String::createWithFormat("%g%s", itemp_totalTtf[jettonArea] / 100000000, U2F("亿").c_str())->getCString());
			}
			else
			{
				ttf->setString(__String::createWithFormat("%.3f%s", itemp_totalTtf[jettonArea] / 100000000, U2F("亿").c_str())->getCString());
			}
		}
		else
		{
			ttf->setString(__String::createWithFormat("%g", itemp_totalTtf[jettonArea])->getCString());
		}
	}
	void MainScene_sicbo::PlaceJettonEffect(int Area, int JessonNumber, bool isMySealt, bool isFirstBet = true)
	{
		//筹码效果
		jettonSprite = Sprite::create(SETPATH("sicbo/Res/gold.png"));
		...
		{
			jettonSprite->setPosition(Vec2(684, 1450));
		}
	...
	//总注下注位置
	Vec2 MainScene_sicbo::JettonTotalPlace(int jettonArea)
	{
		switch (jettonArea)
		{
		case 44:case 45:case 46:case 47:case 48:case 49:
			return(Vec2(0, -36) + myJettonLabel[jettonArea]->getPosition());;
			break;
		case 2:case 3:case 4:case 5:case 6:case 7:
			return(Vec2(0, -24) + myJettonLabel[jettonArea]->getPosition());
			break;
		default:
			return(Vec2(0, -36) + myJettonLabel[jettonArea]->getPosition());
			break;
	...
	//随机一个下注区域的坐标 
	Vec2 MainScene_sicbo::randJettonPos(int jettonArea)
	{
	...
	SCORE MainScene_sicbo::getJettonNumber(int jType)
	{
		SCORE jScore = 0;
		switch (jType)
		{
	...
	int	MainScene_sicbo::getJettonCurType(SCORE jScore)
	{
		int jType = 0;
	...
	void MainScene_sicbo::clearRecordList()
	{
		memset(&jettonProbability, 0, sizeof(jettonProbability));
		record_list->removeAllChildren();
	}
	void MainScene_sicbo::addRecordList(tagGameRecord& obj, int itemp_LaterCount)
	{
		for (int i = 0; i < 3; i++)
		{
		...
		ImageView * recordNumber1 = ImageView::create(SETPATH(__String::createWithFormat("sicbo/onlineFrame/%d.png", obj.cbDiceValue[0])->getCString()));
		recordNumber1->setPosition(ccp(23, 30));
		ImageView * recordNumber2 = ImageView::create(SETPATH(__String::createWithFormat("sicbo/onlineFrame/%d.png", obj.cbDiceValue[1])->getCString()));
		recordNumber2->setPosition(ccp(77, 30));
		ImageView * recordNumber3 = ImageView::create(SETPATH(__String::createWithFormat("sicbo/onlineFrame/%d.png", obj.cbDiceValue[2])->getCString()));
		recordNumber3->setPosition(ccp(130, 30));

		auto record_item = Sprite::create(SETPATH("sicbo/onlineFrame/line3.png"));
		record_item->setAnchorPoint(Vec2::ZERO);

		auto itemBg = Sprite::create(SETPATH("sicbo/onlineFrame/line3.png"));
	...
	//自动跟注勾全部取消掉
	void MainScene_sicbo::automaticJCancel() 
	{
		auto btn_10 = static_cast<CheckBox *>(btn_plane->getChildByName("btn_10"));
		auto btn_20 = static_cast<CheckBox *>(btn_plane->getChildByName("btn_20"));
		auto btn_50 = static_cast<CheckBox *>(btn_plane->getChildByName("btn_50"));
	...
	void MainScene_sicbo::subChangeBanker(CMD_S_ChangeBanker user)
	{
		...
		if (user.wBankerUser == INVALID_CHAIR)
		{
			...
			if (wCurrentBankerState == CurrentBanker_sicbo_Already)
			{
				wCurrentBankerState = CurrentBanker_sicbo_None;
				this->changeBankerButton();
		...
		if (player)
		{
			for (int i = 0; i < banker_list->getItems().size(); i++)
			{
				...
				{
					wCurrentBankerState = CurrentBanker_sicbo_Ready;
					this->changeBankerButton();
					banker_Btn->setTouchEnabled(true);
					return;
				}
	...
	//如果是自己上庄后的界面显示
	void MainScene_sicbo::showUpeerBanker(bool isBanker)
	{
		...
		else
		{
			...
			automaticJ_btn->loadTextures(SETPATH("sicbo/Res/automaticJ_btn_n.png"), "");
			repeatJ_btn->loadTextures(SETPATH("sicbo/Res/repeatJ_btn_n.png"), "");
		}
	}
	
	void MainScene_sicbo::removeBanker(WORD id)
	{
		for (int i = 0; i < banker_list->getItems().size(); i++)
		{
	...
	void MainScene_sicbo::removeBankerSub(WORD id)
	{
		for (int i = 0; i < banker_list->getItems().size(); i++)
		{
			if (banker_list->getItem(i)->getTag() == id)
	...
	//影藏自动跟注的显示
	void MainScene_sicbo::hideContinueJetton(bool istrue)
	{
		if (!isContinueJetton)
		{
			return;
	...
	void MainScene_sicbo::setLabelType(int time, TimeType_sicbo type)
	{
		...
		switch (type)
		{
		case TimeType_sicbo_PlaceJetton:   //下注时间
		{
											   //声音
											   SoundManager::instance()->playEffect(SETPATH("sicbo/sound/game_state.mp3").c_str());
											   armatureBird->setVisible(true);
											   ...
											   if (isBanker)
											   {
												   ...
												   banker_Btn->setTouchEnabled(false);
												   banker_Btn->loadTextureNormal(SETPATH("sicbo/Image/banker_Btn2_s.png"));
												   for (int i = 0; i < 6; i++)
												   {
													   B_CurMoney[i]->setTouchEnabled(false);
													   B_CurMoney[i]->setBright(false);
											   ...
											   else
											   {
												   ...
												   else
												   {
													   automaticJ_label->setVisible(false);
													   jessonPanelBlack->setVisible(false);
													   if (wCurrentBankerState != CurrentBanker_sicbo_Ready)
													   {
														   banker_Btn->loadTextureNormal(SETPATH("sicbo/onlineFrame/banker_Btn1.png"));
													   }
												   }
			...
			break;
		case TimeType_sicbo_Start:
		{
									 //声音
									 SoundManager::instance()->playEffect(SETPATH("sicbo/sound/stop_bet.mp3").c_str());
									 S_circleLight->setVisible(false);
									 armatureBird->setVisible(true);
									 armatureBird->getAnimation()->play("Yxx_lhd_betsstop");
									 timeLable->setString(U2F("本局游戏开奖中"));
									 // timeLabelTop->setVisible(false);
									 for (int i = 0; i < 52; i++)
									 {
			...
			break;
		case TimeType_sicbo_Leisure:
		{
									   for (int i = 0; i < 52; i++)//停止渐变
									   {
									   ...
									   banker_Btn->setTouchEnabled(true);
									   if (wCurrentBankerState != CurrentBanker_sicbo_Ready)//自己不在申请上庄列表中
									   {
										   if (!isBanker)
										   {
											   banker_Btn->loadTextureNormal(SETPATH("sicbo/onlineFrame/banker_Btn1.png"));
	...
	void MainScene_sicbo::userApplyBanker(int seatId)
	{
		...
		if (player)
		{
			...
			auto m_sItempFrame = Sprite::create(SETPATH("sicbo/Res/head2.png"));
			...
			auto nameTTf = LabelTTF::create();
			nameTTf->setString(player->getNickName().c_str());
			nameTTf->setPosition(Vec2(175, 58));
			nameTTf->setAnchorPoint(Vec2(0.0f, 0.5f));
			nameTTf->setColor(Color3B(243, 81, 0));
			nameTTf->setFontSize(20);
			bg->addChild(nameTTf);
			...
			scoreTTf->setString(__String::createWithFormat("%lld", player->getScore())->getCString());
			scoreTTf->setName("scoreTTf");
			scoreTTf->setPosition(Vec2(200, 28));
			scoreTTf->setColor(Color3B(243, 81, 0));
			scoreTTf->setAnchorPoint(Vec2(0.0f, 0.5f));
			scoreTTf->setFontSize(20);
	...
	//复投消息处理
	void MainScene_sicbo::userRepeatPlaceJetton(bool istrue, CMD_S_RepeatBet&  pQueryStorage, bool isFirstBet)
	{
		LONGLONG jetton_repeatNumberVec[52];
	...
	//重复下注
	void MainScene_sicbo::repaetButtonNoifyFial()
	{
		if (isContinueJetton)
		{
			PromptLayer::createObject(U2F("自动跟注中，无法下注").c_str());
	...
	int MainScene_sicbo::getCurrentRate(int type)
	{
		int rateNumber = 0;
		switch (type)
		{
	...
	//继续加注
	bool MainScene_sicbo::continueJetton()
	{
		int itemp_Score = 0;
		...
		if (isContinueJetton)
		{
			if (continueJetton_number != ButtonType_sicbo_automaticJ_Infinite)//如果是无限跟注，不递减次数,递增跟注次数
			{
	...
	//自动下注的显示
	void MainScene_sicbo::showContinueJetton()
	{
		if (continueJetton_number != ButtonType_sicbo_automaticJ_Infinite)
		{
			//可以跟注
			automaticJ_label->setString(__String::createWithFormat(U2F("自动跟注中...(剩余%d轮)").c_str(), continueJetton_number)->getCString());
		}
		...
		if (wCurrentTimeType == TimeType_sicbo_PlaceJetton && wCurrentJettonScore != 0)
		{
			auto btns = (Button *)BgPanel->getChildByName(CurrentButtonName(wCurrentJettonScore));
			btns->stopAllActions();
			btns->runAction(MoveTo::create(0.1f, Vec2(btns->getPosition().x, 54)));
	...
	bool MainScene_sicbo::checkAutoMatic()
	{
		int number = 0;
		if (wCurrentTimeType == TimeType_sicbo_PlaceJetton)//如果不是下注时间自动跟注，那么比较上一轮的押注数目
		{
			for (int i = 0; i < 52; i++)
			{
	...
	void MainScene_sicbo::setMenuState(bool orisMenu)
	{
		auto menuPanel = static_cast<Widget *>(BgPanel->getChildByName("menuPanel"));
		auto menu_btn = static_cast<Button *>(BgPanel->getChildByName("menu_btn"));
		if (orisMenu)
	...
	void MainScene_sicbo::setludanState(bool b_itempBanerMove)
	{
		if (b_itempBanerMove)
		{
			isLudanMove = false;
			ludan_total->runAction(MoveTo::create(0.2f, ludan_total->getPosition() + Vec2((ludan_total->getContentSize().width - 63 + record_btn->getContentSize().width), 0)));
	...
	void MainScene_sicbo::setListState(bool b_itempBanerMove)
	{
		auto list_planeL = static_cast<Widget *>(BgPanel->getChildByName("list_planeL"));
		...
		{
			isBanerMove = false;
			list_planeL->runAction(MoveTo::create(0.2f, list_planeL->getPosition() + Vec2(-(list_planeL->getContentSize().width + 53 - btn->getContentSize().width), 0)));
		}
		else
		{
			isBanerMove = true;
	...
	//结算界面
	void MainScene_sicbo::overLayoutFunc(CMD_S_GameEnd& endData)
	{
		//路单更新
		...
		if (endData.lUserScore > 0)
		{
			...
			for (int i = 0; i < nodeCount; i++)
			{
				...
				//0~1/3s变宽，变扁
				//1/3s~1s还原
				Sprite *spritegold = Sprite::create(SETPATH("sicbo/Res/gold.png"));
	...
	void MainScene_sicbo::onBackPressed()
	{

	}

	void MainScene_sicbo::onJessonTouch(Ref* target, Widget::TouchEventType type)
	{
		Button *btn = (Button *)target;
		if (Widget::TouchEventType::BEGAN == type)
		{
			...
			if (isBanerMove == true)
			{
				if (itemp_tag != ButtonType_sicbo_listplantL_move&&itemp_tag != ButtonType_sicbo_banker &&itemp_tag != ButtonType_sicbo_b && itemp_tag != ButtonType_sicbo_banker_move&&itemp_tag != ButtonType_sicbo_ludan_Frame_move&&itemp_tag != ButtonType_sicbo_ludan_move)
				{
					setListState(isBanerMove);
				}
			}
	...
	string MainScene_sicbo::CurrentButtonName(int currentScore)
	{
		return B_CurMoney[getJettonCurType(currentScore) - 1]->getName();
	}
	int MainScene_sicbo::ButtonNameChangeScore(string currentName)
	{
		for (int i = 0; i < 6; i++)
		{
