...
namespace fruit
{
	...
	bool helpLayer::init()
	{
		...
		auto pRoot = cocostudio::GUIReader::getInstance()->widgetFromJsonFile(SETPATH("fruit/helpLayer.json").c_str());
		CCSize visibleSize = Director::getInstance()->getVisibleSize();
		//pRoot->setAnchorPoint(CCPoint(0.0f, 1.0f));
		...
		auto help_bg = Sprite::create(SETPATH("fruit/rule/rule.png").c_str());
		if (help_bg)
		{
