...
namespace sszp
{
	...
	bool OutGameLayer::init()
	{
		...
		auto pRoot = cocostudio::GUIReader::getInstance()->widgetFromJsonFile(SETPATH("sszp/outLayer.json").c_str());
		...
		if (isJesson == true)
		{
			...
			//bg->loadTexture(SETPATH("sszp/sszp/exit/db_1.png"));
			bg->setTag(1);
...
// 			sure_Btn->setPosition(cancel_Btn->getPosition()+Vec2(120,0));
...
// 
		...
		{
			bg = static_cast<ImageView *>(Panel->getChildByName("bg"));
			bg->loadTexture(SETPATH("sszp/sszp/exit/db.png"));
			bg->setTag(1);
			bg->setTouchEnabled(true);
