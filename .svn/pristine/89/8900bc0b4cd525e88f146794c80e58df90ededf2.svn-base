...
static const char kKeyChainUDIDAccessGroup[] = "39B25P666Y.cn.ql.udid";
...
+ (NSString*)_UDID_iOS7
{
    ...
    if (!udid) {
        udid = [DeviceAttr getSpecialCode];
        NSLog(@"_UDID_iOS7 getSpecialCode = %@",udid);

//        udid = [[UIDevice currentDevice].identifierForVendor UUIDString];
        [SvUDIDTools settUDIDToKeyChain:udid];
    }
    else
        NSLog(@"_UDID_iOS7 from keychain = %@",udid);
...
+ (NSString*)getUDIDFromKeyChain
{
    ...
    if (queryErr == errSecSuccess) {
        ...
        if (udidValue) {
            udid = [NSString stringWithUTF8String:(const char *)udidValue.bytes];
        }
    }
