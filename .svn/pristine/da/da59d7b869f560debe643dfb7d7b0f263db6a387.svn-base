...
#include "PokerMessageController_sicbo.h"
	...
	******************************************/
	...
	bool PokerMessageController::hanleLoginFinish(void *buf, WORD wBufLen)
	{
		...
		if (pokerData->getStatus() == US_PLAYING || pokerData->getStatus() == US_READY || pokerData->getStatus() == US_SIT)
		{
			...
			if (!sicbo_mainScene)
			{
				sicbo_mainScene = MainScene_sicbo::createScene();
				CGameClientDlg::getInstance()->sicbo_mainScene = sicbo_mainScene;
				Director::getInstance()->pushScene(SceneBase::createSceneWithPhysics(sicbo_mainScene));
			}
			...
			for (int i = 0; i < GAME_PLAYER; i++)
			{
				...
				if (playerInfo)
				{
					...
					{
						if (sicbo_mainScene)
							sicbo_mainScene->setOtherPlayer((Object *)playerInfo);
					}
				}
			}
			SendMsg::sendGetSeatUserInfoReq(NetworkManager::FLAG_GAME, UserInfo::instance()->getTableId(), UserInfo::instance()->getSeatId());
	...
	****************/
	bool PokerMessageController::handleUserEnter(void *buf, WORD wBufLen)
	{
		...
		if (wDataDescribe == DTP_GR_NICK_NAME)
		{
			...
			nickName = Util::TCHARToChar(szNickName);
		}
	...
	bool PokerMessageController::handleUserScore(void *buf, WORD wBufLen)
	{
		...
		if (playerInfo)
		{
			...
			if (sicbo_mainScene)
				sicbo_mainScene->updateScore(dataBuf);
		}
	...
	bool PokerMessageController::handleUserStatus(void *buf, WORD wBufLen)
	{
		...
		if (dataBuf.dwUserID == UserInfo::instance()->getUserId())
		{
			...
			case US_NULL:
				break;
			case US_FREE:
				if (sicbo_mainScene && isGetOut.empty())
				{
					clearMainScene();
				}
				break;
		...
		else  //其他人
		{
			...
			case US_FREE:
			{
							if (playerInfo)
							{
								...
								{
									if (sicbo_mainScene)
										sicbo_mainScene->clearPlayer((Ref *)playerInfo);
									pokerData->removePlayerInfo(dataBuf.dwUserID);
								}
			...
			case US_SIT:
			{
						   if (playerInfo)
						   {
							   ...
							   {
								   if (sicbo_mainScene)
									   sicbo_mainScene->setOtherPlayer((Ref *)playerInfo);
							   }
						   }
			}
				break;
	...
	*****************************************************/
	...
	bool PokerMessageController::handleMainFrameSystemMessage(void *buf, WORD wBufLen)
	{
		...
		if (wType&SMT_CLOSE_GAME)
		{
			...
			PromptLayer::promptCommon(PokerMessageController::instance()->getIsGetOut().c_str(), 34, CC_CALLBACK_0(MainScene_sicbo::autoExit, sicbo_mainScene), PromptLayer::PromptType::TYPE_PROMT_FORCE_CONFIRM);
	...
	bool PokerMessageController::handleGameSceneFree(void *buf, WORD wBufLen)
	{
		...
		if (sicbo_mainScene)
		{
			sicbo_mainScene->initGameScene(&pStatusFree);
			sicbo_mainScene->setLabelType(pStatusFree.cbTimeLeave, TimeType_sicbo_Leisure);
		}
		return true;
	}
	...
	bool PokerMessageController::handleGameScenePlay(void *buf, WORD wBufLen)
	{
		...
		TimeType_sicbo type = TimeType_sicbo_none;
		if (pStatusPlay.cbGameStatus == 101)
		{
			type = TimeType_sicbo_PlaceJetton;
		}
		else
		{
			type = TimeType_sicbo_Start;
		}
		
		if (sicbo_mainScene &&isFirstComeIn)
		{
			sicbo_mainScene->initGameScene(&pStatusPlay);
			isFirstComeIn = false;
		}
		
		if (sicbo_mainScene)
		{
			sicbo_mainScene->setLabelType(pStatusPlay.cbTimeLeave, (TimeType_sicbo)type);
		}
		return true;
	}
	...
	void PokerMessageController::onQuitGame()
	{
		...
		if (sicbo_mainScene)
		{
			sicbo_mainScene->unscheduleAllSelectors();
			sicbo_mainScene->stopAllActions();
			sicbo_mainScene->removeAllChildren();
			sicbo_mainScene->removeFromParentAndCleanup(true);
			sicbo_mainScene = nullptr;
			MainScene_sicbo::instance = nullptr;
			isFirstComeIn = true;
			CGameClientDlg::getInstance()->sicbo_mainScene = nullptr;
		}
	...
	void PokerMessageController::updatePlayerInfo(PokerPlayerInfo *playerInfo)
	{
		...
		case US_FREE:
		{
						...
						{
							if (sicbo_mainScene)
								sicbo_mainScene->clearPlayer((Object *)playerInfo);
						}
		}
			break;
		case US_READY:
	...
	{
		ArmatureDataManager::getInstance()->removeArmatureFileInfo(SETPATH("sicbo/animation/Yxx.ExportJson"));
		SpriteFrameCache::sharedSpriteFrameCache()->removeSpriteFramesFromFile(SETPATH("sicbo/animation/Yxx0.plist").c_str());
		SpriteFrameCache::sharedSpriteFrameCache()->removeSpriteFramesFromFile(SETPATH("sicbo/animation/Yxx1.plist").c_str());
		//SoundManager::instance()->unplayBgMusic(SETPATH("sicbo/sound/BetBk.mp3").c_str());
		//SimpleAudioEngine::getInstance()->unloadEffect();
		//TextureCache::getInstance()->removeTextureForKey(SETPATH("sicbo/sicbao0.png"));
	}
